    ...
    for (var j = 0; j < Object.keys(m.tranlyze.key_b).length; j++) {
      for (var k = 0; k < m.tranlyze.key_b[Object.keys(m.tranlyze.key_b)[j]].length; k++) {
        if (Object.keys(m.tranlyze.key_b)[j] == "en2") {
          ...
          } else if (input[i].match(/[ᅡ-ᅵ]/) != null && input[i + 1] == "ᄋ" && input[i + 2] == "ᅨ") { // 모음 뒤에 ㅖ 올 때 붙임표. 붙임표를 넣지 않으면 ㅆ 받침과 헷갈릴 수 있기 때문이다.
          ...
          } else if (input[i].match(/[ᅣᅪᅮᅯ]/) != null && input[i + 1] == "ᄋ" && input[i + 2] == "ᅢ") { // 모음 ㅑ, ㅘ, ㅜ, ㅝ 뒤에 '애'가 올 때 붙임표. 붙임표를 넣지 않으면 ㅒ/ㅙ/ㅟ/ㅞ와 헷갈릴 수 있기 때문이다.
          ...
          } else if (input[i].match(/[ᄂ-ᄄᄆ-ᄇᄊᄌᄍᄏ-ᄒ]/) != null && input[i + 1] == "ᅡ" && input[i + 2] != "ᄋ") { // 모음 ㅏ 붙을 때 생략. '나', '다', '마', '바', '자', '카', '타', '파', '하' 다음에 모음이 올 경우는 혼동의 우려가 있으므로 약자로 쓰지 않는다.
