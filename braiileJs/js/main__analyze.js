...
function improve_jp(input) {
  var text = input;
  // text = text.replace(new RegExp("(?<=[ア-ヿ])[ツ](?=[ア-ヿ])(?!([ア-ヿ][\u3099])|[ア-オヤユヨ]|\s)"), "ッ"); //촉음 x(?<=y) x(?<!y) may couse error on safari
  text = text.replace(new RegExp("([ア-ヿ])[ツ](?=[ア-ヿ])(?!([ア-ヿ][\u3099])|[ア-オヤユヨ]|\s)", "g"), "$1ッ"); //촉음
  text = text //요음
    ...
    .replace(new RegExp("(([キシチニヒミリ])|([キシチヒ][\u3099]))[ヤ]", "g"), "$1ャ")
    .replace(new RegExp("(([キシチニヒミリ])|([キシチヒ][\u3099]))[ユ]", "g"), "$1ュ")
    .replace(new RegExp("(([キシチニヒミリ])|([キシチヒ][\u3099]))[ヨ]", "g"), "$1ョ");
  return text;
}
